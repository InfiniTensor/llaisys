...
__global__ void addKernel(T *c, const T *a, const T *b, size_t numel) {
    ...
    if (idx < numel) {
        float af = to_float_cuda(a[idx]);
        float bf = to_float_cuda(b[idx]);
        c[idx] = from_float_cuda<T>(af + bf);
    }
}
...
void add(std::byte *c, const std::byte *a, const std::byte *b, llaisysDataType_t type, size_t size) {
    ...
    case LLAISYS_DTYPE_BF16:
        return add_<bf16_t_cuda>(c, a, b, size);
    case LLAISYS_DTYPE_F16:
        return add_<fp16_t_cuda>(c, a, b, size);
    default:
