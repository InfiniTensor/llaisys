...
__global__ void rmsNormKernel(T *out, const T *in, const T *weight, float eps, size_t hidden_dim) {
    ...
    for (size_t i = tid; i < hidden_dim; i += blockDim.x) {
        float val = to_float_metax(in_row[i]);
        local_sum += val * val;
    }
    ...
    for (size_t i = tid; i < hidden_dim; i += blockDim.x) {
        float val = to_float_metax(in_row[i]);
        float w = to_float_metax(weight[i]);
        float result = (val / rms) * w;
        out_row[i] = from_float_metax<T>(result);
    }
}
...
void rms_norm(std::byte *out, const std::byte *in, const std::byte *weight,
    ...
    case LLAISYS_DTYPE_BF16:
        return rms_norm_<bf16_t_metax>(out, in, weight, eps, batch_size, hidden_dim);
    case LLAISYS_DTYPE_F16:
        return rms_norm_<fp16_t_metax>(out, in, weight, eps, batch_size, hidden_dim);
    default:
