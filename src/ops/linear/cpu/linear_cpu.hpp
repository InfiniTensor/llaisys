...
// 意义：指向权重矩阵的内存首地址。
// 形状：逻辑形状为 [N, K]。
// 注意：公式中是W^T（转置），但在内存中W是按[N, K]连续存储的。
// 代码通过逻辑索引weight[j*K+k]实现了在不物理转置内存的情况下进行转置乘法。
...
// 对应关系：它等于权重矩阵 W 的行数，也是偏置 b 的长度，以及输出Y的列数。

// size_t K
// 意义：输入特征的维度（Input Features）。
// 对应关系：它等于输入X的列数，同时也必须等于权重矩阵W的列数（矩阵乘法的内维必须匹配）。
} // namespace llaisys::ops::cpu