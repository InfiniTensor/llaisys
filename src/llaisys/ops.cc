...
__C {
    ...
    void llaisysSample(llaisysTensor_t next_token_id, llaisysTensor_t logits, float temperature, int top_k, float top_p) {
        llaisys::ops::sample(next_token_id->tensor, logits->tensor, temperature, top_k, top_p);
    }
}
