...
__C {
    ...
    int64_t llaisysQwen2ModelInfer(struct LlaisysQwen2Model * model, int64_t * token_ids, size_t ntoken, llaisysTensor_t *kcache, llaisysTensor_t *vcache, size_t past_len) {
        ...
        for (size_t i = 0; i < nlayer; i++) {
            ...
            q_tensor = tensorView(q_tensor, q_tensor_reshape_shape, 3);
            ...
            k_tensor = tensorView(k_tensor, k_tensor_reshape_shape, 3);
            ...
            if (kv_cache_used) {
                ...
                // kcache [max_seq, nkvh, d], vcache [max_seq, nkvh, dv]
                k_for_attn = tensorSlice(kcache[i], 0, 0, past_len + seqlen);
                v_for_attn = tensorSlice(vcache[i], 0, 0, past_len + seqlen);
            } else {
                size_t v_tensor_reshape_shape[3] = {seqlen, nkvh, dh};
                v_for_attn = tensorView(v_tensor, v_tensor_reshape_shape, 3);
            ...
            output_self_attn_tensor = tensorView(output_self_attn_tensor, output_self_attn_tensor_shape, 2);
